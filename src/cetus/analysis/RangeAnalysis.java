...
public class RangeAnalysis extends AnalysisPass
{
    ...
    static {
        ...
        //default range analysis level is intraprocedural
        option = RANGE_INTRA;
    ...
    */
    ...
    public void start() {
        ...
        // Considers the specified IR class type as one whose child nodes are 
        // not visited during iteration.
        ...
        while (iter.hasNext()) {
            ...
            //testSubstituteForward(procedure, ranges);
            //filter incorrelate statements
            getArrayAccessRange(ranges);
    ...
    // %add yangzhen 2015-11-09    
    private boolean validStatement(Statement stmt,Set<Expression> condExpression) {
        boolean valid=true;
        if(stmt instanceof DeclarationStatement || stmt instanceof CompoundStatement)
            valid=false;

        if(stmt instanceof WhileLoop || stmt instanceof DoLoop) {
            Loop loop=(Loop)stmt;
            condExpression.add(loop.getCondition());
        }
        //ForLoop statement have for elements
        // init statement (we ignore it temporarily)
        // condition expression
        // step expression
        // body statement
        if(stmt instanceof ForLoop) {
            ForLoop for_loop=(ForLoop)stmt;
            condExpression.add(for_loop.getCondition());
            condExpression.add(for_loop.getStep());
        }

        if(stmt instanceof IfStatement) {
            IfStatement if_stmt=(IfStatement)stmt;
            condExpression.add(if_stmt.getControlExpression());
        }

        if(stmt instanceof SwitchStatement) {
            SwitchStatement switch_stmt=(SwitchStatement)stmt;
            condExpression.add(switch_stmt.getExpression());   
        }

        if(stmt instanceof Case) {
            Case case_stmt=(Case)stmt;
            condExpression.add(case_stmt.getExpression());   
        }

        return valid;
    }

    // %add yangzhen 2015-11-10
    // return the corresponding statement
    // private Statement validExpressionFilter(Set<Expression> condExpression) {
    //     //global array access
    //     DFIterator<Traversable> iter=new DFIterator<Traversable>(condExpression);
    //     iter.pruneOn(ArrayAccess.class);
    //     while(iter.hasNext()) {
    //         Traversable o=iter.next();
    //         if(o instanceof ArrayAccess) {
    //             ArrayAccess array_access=(ArrayAccess)o;
    //             Set<Symbol> acc_symbols=SymbolTools.getAccessedSymbols(o);
    //             for(Symbol symb:acc_symbols) {
    //                 if( SymbolTools.isArray(symb) && SymbolTools.isGlobal(symb) ) 
    //                     return condExpression.getStatement();
    //             }
    //         }
    //     }
    //     return null;
    ...
    // %add yangzhen 2015-11-10
    private void bottomUpStatementFilter(Traversable parent,Map<Statement,RangeDomain> result,
        Set<Traversable> dup_stmt) {
        while(true) {
            if(dup_stmt.contains(parent) || parent instanceof Procedure)
                break;
    ...
    // %add yangzhen 2015-11-10
    // return the nearest statement 
    private Statement globalArrayAccessAnalysis(Traversable root) {

        Statement nearest_stmt=null;
        DFIterator<Expression> iter=new DFIterator<Expression>(root,Expression.class);
        iter.pruneOn(ArrayAccess.class);
        while (iter.hasNext()) {
            Expression o = iter.next();
            if(o instanceof ArrayAccess ) {
                ArrayAccess array_access=(ArrayAccess)o;
                nearest_stmt=IRTools.getAncestorOfType(o,Statement.class);
                //traverse the array access
                // Set<Symbol> acc_symbols=SymbolTools.getAccessedSymbols(o);
                // for(Symbol symb:acc_symbols) {
                //     if( SymbolTools.isArray(symb) && SymbolTools.isGlobal(symb) ) {
                //         nearest_stmt=IRTools.getAncestorOfType(o,Statement.class);
                //         break;
                //     }
                // }// Symbol symb:acc_symbols
            }// o instanceof ArrayAccess
            if(nearest_stmt!=null) break;
        }
        return nearest_stmt;
    }

    // %add yangzhen 2015.11.03
    private Map<Statement,RangeDomain> getArrayAccessRange(
        Map<Statement,RangeDomain> ranges) {
        // result
        Map<Statement,RangeDomain> result=new HashMap<Statement,RangeDomain>();
        // store the unuseful statement 
        Set<Traversable> duplicate_statement=new HashSet<Traversable>();
        Set<Expression> condition_expression=new HashSet<Expression>();

        for(Statement stmt:ranges.keySet()) {
            //indicate that that array points to malloc location
            boolean flag=false;
            RangeDomain rd=ranges.get(stmt);

            if (!validStatement(stmt,condition_expression) || duplicate_statement.contains(stmt)) {
                if(stmt instanceof CompoundStatement)
                    bottomUpStatementFilter(stmt,result,duplicate_statement);
                continue;
            }
...
System.out.println("====================domain:"+domain+"==================");
                if(domain!=null) {
                    for(Symbol s : domain.keySet()) {
                        if(!SymbolTools.isPointer(s))
                            continue;

                        for(PointsToRel rel : domain.get(s)) {
                            //this indicates that there are pointers shown before current 
                            //statement and we need to guarantee that one of the pointer symbols
                            //in current statement
                            if(IRTools.containsSymbol(stmt,s) && 
                                (rel.getPointedToSymbol().toString().contains("calloc") ||
                                ...
                                result.put(stmt,ranges.get(stmt));

                                bottomUpStatementFilter(stmt.getParent(),result,duplicate_statement);
                                flag=true;
                                break;
                            }
                        }
            ...
            Statement nearest_stmt=globalArrayAccessAnalysis(stmt);
            if(nearest_stmt!=null)
                result.put(nearest_stmt,ranges.get(nearest_stmt));
        }
System.out.println("###############original size:"+ranges.size()+"################");
System.out.println("###############result size:"+result.size()+"################");
        for(Statement s:result.keySet()) {
            System.out.println("###############statement:"+s+"###############");
            if( !(IPPointsToAnalysis.getPointsToRelations(s) instanceof NullDomain) &&
                !(IPPointsToAnalysis.getPointsToRelations(s) instanceof PointsToDomain.Universe))
            {
                PointsToDomain domain=(PointsToDomain)IPPointsToAnalysis.getPointsToRelations(s);
                System.out.println("###############pointsto domain:"+domain+"###############");    
            }
            
            System.out.println("##############range domain"+result.get(s)+"#################");
        }

        duplicate_statement.clear();
        return result;
    }

    // %add yangzhen 2015-11-14
    private void arrayParameterAnalysis() {
        initializeData();
    ...
    // %add yangzhen 2015-11-14
    // keep the formal parameters to their equivalent arguments for every call site.
    private Map<Procedure,
                Map<FunctionCall,
    ...
    private void initializeData() {
        ...
        param_to_args=new IdentityHashMap<Procedure,
                                          Map<FunctionCall,
                                              Map<Symbol,Expression>>>();
        for(Iterator<IPANode> iter=callgraph.topiterator();iter.hasNext();) {

            //Class IPANode represents workspace for a procedure
            IPANode node=iter.next();
            Procedure procedure=node.getProcedure();
            param_to_args.put(procedure,new IdentityHashMap<FunctionCall,
                                                            Map<Symbol,Expression>>());
            //the set of CallSite objects that calls the current node.
    ...
    private void processCallee(IPANode node) {
        //make conservative decisions for variable argument list
        if(node.containsVarArg())
            return ;

        for(CallSite calling_site:node.getCallingSites()) {
            Map<Symbol,Expression> equiv=param_to_args.get(node.getProcedure()).get(
                calling_site.getFunctionCall());
    ...
    // %add yangzhen 2015-11-14
    // analyze the relation between the parameters and the argument
    // return the equivalent representation of the formal parameter in terms
    // of caller's expression.
...
//System.out.println("++++++++++normalize points to:"+PointsToAnalysis.normalizePointsTo(arg)+"+++++++++");

            arg=processBinary((BinaryExpression)arg);
        }
        //pointers using ptr[i][j] access don't have array specifiers
        Symbol arg_symbol=SymbolTools.getSymbolOf(arg);

        if(!(arg instanceof Identifier || arg instanceof UnaryExpression ||
        ...
        // with address_of operator
            ...
            if(op==UnaryOperator.ADDRESS_OF) {
                if(expr instanceof ArrayAccess) {
                    ...
                    Expression last_index=array.getIndex(array.getNumIndices()-1);
...
//System.out.println("++++++++++ret:"+ret+"+++++++++++");
    ...
    // analysis the relationship between the formal parameter and the array arguments
    ...
    // %add yangzhen 2015-11-14
    // test if the lhs/rhs pair represents apair arithmetic with base and offset
    ...
    private void addAssertions(
        ...
        while (iter.hasNext()) {
            ...
            if (rd != null && rd.size() > 0) {
                ...
                stmt.annotateBefore(
                        new CodeAnnotation("assert(" + test + ");//line:"+stmt.where()));
            }
        }
    }
    ...
    */
    public static Map<Statement, RangeDomain> getRanges(SymbolTable symtab) {
        ...
        //Returns the nearest ancestor object of the given traversable object that
        ...
        if (symtab instanceof TranslationUnit) {
            ...
            return ret;
        //symtab belongs to the procedure
