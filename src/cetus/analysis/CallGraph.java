...
public class CallGraph {
    ...
    public class Node {
        ...
        // %update yangzhen 2015-11-05
        // can reach the pthread_create from bottom to up
        private boolean isAvailableFromPthreadCreate;
        ...
        public Node(Procedure proc) {
            ...
            isAvailableFromPthreadCreate=false;
        }

        public void setAvailableFromPthreadCreate(boolean b) {
            isAvailableFromPthreadCreate=b;
        }
        public boolean availableFromPthreadCreate() {
            return isAvailableFromPthreadCreate;
        }// %update end
    ...
    */
    public CallGraph(Program program) {
        ...
        //mapping the procedure and it's name
        Map<String,Procedure> procedureName=new HashMap<String,Procedure>();
        ...
        while (iter.hasNext()) {
            ...
            procedureName.put(proc_name,proc);
        }

        Expression entry_function=null;
        ...
        while (fc_iter.hasNext()) {
            ...
            //find the function call `pthread_create`
            if(call.getName().toString().contains("pthread_create")) {
                //get the second argument,which is the entry function
                entry_function=call.getArgument(2);
                Procedure proc=procedureName.get(entry_function.toString());
                //this is the first procedure node can reach the pthread_create
                if(proc!=null) {
                    callgraph.get(proc).setAvailableFromPthreadCreate(true);
                    //procedureName is unuseful
                    procedureName.clear();
                }
            }
            ...
            if (l_calledproc != null) {
                ...
                // called procedure can reach the pthread_create
                if(currproc!=null && calledproc!=null && currproc.availableFromPthreadCreate())
                    calledproc.setAvailableFromPthreadCreate(true);
            }
        }
    }
...
p.print(" [reach_pthread_create:"+node.availableFromPthreadCreate()+"]");
            p.print(" -> { ");
    ...
    private void topologicalSort(Procedure proc,
        ...
        for (Procedure callee : (ArrayList<Procedure >)node.getCallees()) {
            if (unvisited.contains(callee)) {
                if(node.availableFromPthreadCreate())
                    callgraph.get(callee).setAvailableFromPthreadCreate(true);
